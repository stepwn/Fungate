...
function fungate_enqueue_tinymce_plugin() {
    if (current_user_can('edit_posts') && current_user_can('edit_pages')) {
        add_filter('mce_external_plugins', 'fungate_add_fungate_tinymce_plugin');
        add_filter('mce_buttons', 'fungate_register_fungate_tinymce_button');
    }
}
add_action('admin_head', 'fungate_enqueue_tinymce_plugin');

function fungate_add_fungate_tinymce_plugin($plugin_array) {
    $plugin_array['fungate_button_script'] = plugins_url('../js/fungate-tinymce-plugin.js', __FILE__);
    return $plugin_array;
}

function fungate_register_fungate_tinymce_button($buttons) {
...
window.addEventListener('message', function(event) {
    if (typeof event.data === 'string' && event.data.startsWith('[fungate')) {
        ...
        function fungate_getRandomColor() {
    ...
    var randomColor = fungate_getRandomColor();
...
function fungate_strip_out_shortcode_junk($content) {
...
add_filter('content_save_pre', 'fungate_strip_out_shortcode_junk');

function fungate_restore_shortcode_to_div_tinymce($content) {
    ...
    function fungate_replace_shortcode($matches) {
        $randomColor = fungate_getRandomColor();
    ...
    $content = preg_replace_callback($pattern, 'fungate_replace_shortcode', $content);

    return $content;
}

add_filter('content_edit_pre', 'fungate_restore_shortcode_to_div_tinymce');
